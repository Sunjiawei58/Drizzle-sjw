...
int df::s_Object::eventHandler(const s_Event * p_e)
{
	return 0;
}
...
void s_Object::draw()
{
	...
	int index = getSpriteIndex();
	  // 3.17 question. why I cannot get the index. what is the index for. and where
//	LogManager2::getInstance().writeLog("pos is %d,%d", pos.getX(), pos.getY());
	//Position p(20, 20);
	
	s_GraphicsManager::getInstance().drawFrame(pos, p_sprite->getFrame(index), sprite_center, p_sprite->getColor());
	
	//s_GraphicsManager::getInstance().swapBuffers();
