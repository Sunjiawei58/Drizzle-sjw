...
int s_GraphicsManager::startUp()   
{
	...
	sf::Text text;
	text.setFont(font);
	text.setString("Hello, Sample Thing");
...
int df::s_GraphicsManager::drawCh_net(Position world_pos, char ch, s_Color color,unsigned int time_counter) const
{
	graphicsPacketCh g_packCh;
	g_packCh.color = color;
	g_packCh.content = ch;
	g_packCh.x = world_pos.getX();
	g_packCh.y = world_pos.getY();
	g_packCh.len = sizeof(g_packCh);
	g_packCh.type = 0;  // draw ch

	g_packCh.timeCounter = time_counter;
	
	s_NetworkManager::getInstance().df_send(static_cast<graphicsPacketCh *>(&g_packCh), g_packCh.len); // send graphics to client
	return 0;
}
int s_GraphicsManager::swapBuffers()
{
	if (p_window == NULL)
	{
	...
	p_window->display();
	s_GraphicsManager::getInstance().screenshot(1);
...
int s_GraphicsManager::drawString(Position pos, std::string str, Justification just, s_Color color) const
{
	...
	for (int i = 0; i < str.size(); i++)
	{
		...
		drawCh_net(tem_pos, str[i], color, s_timeWarp::getInstance().current_time_counter);
	}
	return 1;   // 1 for ok
}
...
int s_GraphicsManager::drawFrame(Position world_pos, s_Frame frame,bool sprite_centered,s_Color color)
{
	...
	for (int y = 0; y < frame.getHeight(); y++)
	{
		...
		//	drawCh(tmp_pos, str[y*frame.getWidth() + x], color);
			drawCh_net(tmp_pos, str[y*frame.getWidth() + x], color, s_timeWarp::getInstance().current_time_counter);
